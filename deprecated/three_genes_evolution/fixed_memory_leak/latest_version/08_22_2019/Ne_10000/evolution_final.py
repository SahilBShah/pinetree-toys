...
export_csv = all_sos_dataframe.to_csv("~/pinetree-toys/three_genes_evolution/fixed_memory_leak/latest_version/08_22_2019/Ne_10000/all_sos_data.tsv", index=False)
sos_list.remove(60000)
sos_dataframe = pd.DataFrame(data=sos_list, columns=["Sum_of_Squares"])
export_csv = sos_dataframe.to_csv("~/pinetree-toys/three_genes_evolution/fixed_memory_leak/latest_version/08_22_2019/Ne_10000/sos_data.tsv", index=False)
sos_iter_dataframe = pd.DataFrame(data=sos_iter_list, columns=['Iteration'])
export_csv = sos_iter_dataframe.to_csv('~/pinetree-toys/three_genes_evolution/fixed_memory_leak/latest_version/08_22_2019/Ne_10000/sos_iter_data.tsv', index=False)
