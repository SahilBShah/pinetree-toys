...
getwd()
setwd("/home/wilkelab/pinetree-toys/three_genes_evolution/second_10000_run")

data <- read.table("best_three_genes_replicated.tsv", header = TRUE)
...
data2 <- read.table("best_three_genes_replicated.tsv", header = TRUE)
...
data3 <- read.table("best_three_genes_replicated.tsv", header = TRUE)
...
data <- read.table("three_genes_replicated.tsv", header = TRUE)
data <- filter(data3, species == c("proteinX", "proteinY", "proteinZ"))
