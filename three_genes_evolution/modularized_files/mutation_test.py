...
def test_mutation(df, genome_tracker, i):
    ...
    if genome_tracker.loc[('f_new'), ('value')] > genome_tracker.loc[('f_old'), ('value')]:
        mutation_accepted.accept_mutation(df, genome_tracker, i)
    else:
        #Calculate fitness of new mutation
        probability = fitness_score.calc_fitness(genome_tracker.loc[('f_new'), ('value')], genome_tracker.loc[('f_old'), ('value')])
