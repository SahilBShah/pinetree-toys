...
setwd("/home/sahil/pinetree-toys/three_genes_evolution/fixed_memory_leak/latest_version/")
...
generation_data <- read.table("sos_iter_data.tsv", header=TRUE)
all_sos_data <- cbind(all_sos_data, generation_data)
#all_sos_data$ID <- seq.int(nrow(all_sos_data))
all_sos_line_plot <- ggplot(all_sos_data, aes(x=Iteration, y=Sum_of_Squares)) + geom_point(stat="identity") + geom_line(stat="identity") + ggtitle("All Sum of Squares Values") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black"))
sos_plots <- grid.arrange(all_sos_line_plot, nrow=2, top="Sum of Squares Data")
ggsave("/home/sahil/pinetree-toys/three_genes_evolution/fixed_memory_leak/latest_version/sos_graphs.pdf", sos_plots, width=20, height=10)
```
## R Markdown
...
ggsave("/home/sahil/pinetree-toys/three_genes_evolution/fixed_memory_leak/latest_version/gen_graphs.pdf", gen_plots, width=20, height=10)
...
best_files <- list.files("/home/sahil/pinetree-toys/three_genes_evolution/fixed_memory_leak/latest_version", pattern="three_genes_replicated_")
...
ggsave("/home/sahil/pinetree-toys/three_genes_evolution/fixed_memory_leak/latest_version/evolution_graphs.pdf", evo_plot, width=30, height=30, limitsize=FALSE)
